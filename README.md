...
In every public totals payload (`get_nutrition_summary`, `get_goal_progress`, `get_trends`, and the log/update-meal progress payloads) the four core macros (`calories`, `protein_g`, `carbs_g`, `fat_g`) are nullable: a total is `null` only when no meal in the selection has a calculated value for that nutrient — pending calculations are never coalesced to `0`, while an explicitly stored `0` stays `0`. Each payload also carries the integer presence counts `meals_total` and `meals_calculated` so a partial sum over a mixed selection is never mistaken for a complete one. Range averages keep the historical every-logged-day denominator but are `null` when no day in the window carries the nutrient. Text output renders a null total as `no data yet` (never `0%` or `NaN`), the widgets render `—`/`no data yet` for null and `0` only for a real zero, and the `export_meals` CSV keeps empty cells for missing values.
